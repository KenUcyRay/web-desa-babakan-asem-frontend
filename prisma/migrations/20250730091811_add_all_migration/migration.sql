...
CREATE TABLE `users` (
    ...
    `role` ENUM('REGULAR', 'ADMIN', 'PKK', 'BUMDES', 'KARANG_TARUNA', 'BPD', 'CONTRIBUTOR') NOT NULL,
...
CREATE TABLE `agendas` (
    ...
    `is_published` BOOLEAN NOT NULL DEFAULT false,
    `published_at` DATETIME(3) NULL,
    `category` ENUM('REGULAR', 'PKK', 'KARANG_TARUNA', 'DPD', 'BPD') NOT NULL DEFAULT 'REGULAR',
    `created_at` DATETIME(3) NOT NULL DEFAULT CURRENT_TIMESTAMP(3),
    `updated_at` DATETIME(3) NOT NULL,
...
CREATE TABLE `members` (
    ...
    `organization_type` ENUM('GOVERNMENT', 'PKK', 'KARANG_TARUNA', 'DPD', 'BPD') NOT NULL,
